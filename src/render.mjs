...
const closing = (paras) => {
  ...
  y = startY
  for (const { p, ls } of lines) for (const l of ls) { draw(l, blockX); y -= p.cls === 'lead' ? 45 : 15 }
}
...
for (const s of body) {
  ...
  for (const [i, p] of s.paragraphs.entries()) {
    if (p.cls === 'subheading') {
      ...
      y -= 18
      for (const l of wrap(p.text, F.bold, SIZE, WIDTH)) { draw(l, PAGE.margin, F.bold); y -= SINGLE }
      y -= 4
    } else if (p.cls === 'cite') {
      ensure(leadIn(s.paragraphs, i))
