...
- The statement of the problems has one lettered section per claim the person chose, with the regulation in italics under its heading. Under it, **say what the problem is** in one to three short paragraphs — what the District did or failed to do, with the dates, figures and words from the documents that make the point — and end with the paragraphs of the chronology that carry the rest: "(paragraphs [#consent], [#delay] and [#report])". A claim that is only a list of paragraph numbers says nothing, and the check refuses it; so does restating the whole chronology under each claim. Point to a handful of paragraphs, not twenty, and point to paragraphs only here.
...
- The proposed resolution: each remedy names its own figures — the weeks the service log records with no instruction, the minutes a week the IEP requires, the sessions a report records as owed — and the document that gives them. Never send the reader to a paragraph number for a remedy.
