...
Then, in Claude Code: *"Draft a due process complaint from the documents in `my-case/`. The state is California and I'm filing pro se."* The skill asks its remaining questions in two batches — once before reading (state, who signs, filing date) and once after the readings are confirmed (the four statements of fact, the claims, the relief, the chronology). Anything you answer up front — including "confirm every reading that checks out against the page; I'll review the sources" — is taken and not asked again, so an attorney who supplies everything in the first message gets a complete run with no questions.
